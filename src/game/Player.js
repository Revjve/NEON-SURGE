import { ARENA, PLAYER, PALETTE, BASE_STATS } from '../config.js';
...
export class Player {
  ...
  update(dt, input, aimX, aimY, maxSpeed = BASE_STATS.moveSpeed) {
    this.time += dt;
    ...
    this.vx += (m.x * maxSpeed - this.vx) * k;
    this.vy += (m.y * maxSpeed - this.vy) * k;
    this.x += this.vx * dt;
    this.y += this.vy * dt;
    ...
    this.thrust = clamp(speed / maxSpeed, 0, 1);
  ...
  /**
   * Fire volleys at the build's rate. Each volley fans `stats.projectiles` bolts around
   * the heading; `emit(x, y, angle)` spawns one bolt. Returns the number of volleys fired.
   */
  tryFire(dt, firing, stats, emit) {
    this.cooldown -= dt;
    ...
    // Fan width grows with the bolt count but stays readable.
    const spread = n > 1 ? Math.min(0.9, 0.11 * (n - 1) + 0.05) : 0;
    let volleys = 0;
    while (this.cooldown <= 0) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        const lateral = t * Math.min(26, 8 * (n - 1));
        const a = this.heading + t * spread + (Math.random() - 0.5) * 1.6 * DEG;
        emit(this.x + ca * nose - sa * lateral, this.y + sa * nose + ca * lateral, a);
      }
      this.fireHeat = 1;
      this.barrel ^= 1;
      if (volleys > 2) break;
  ...
  draw(layer, time, shieldCharges = 0) {
    ...
    if (shieldCharges > 0 && this.invuln <= 0) {
      // Deflector charges: a faint rotating hex per charge.
      for (let i = 0; i < shieldCharges; i++) {
        layer.sprite('shield', this.x, this.y, time * (0.9 + i * 0.5) * (i % 2 ? -1 : 1), r * (1.5 + i * 0.22), PALETTE.shield, 0.55 * appear);
      }
    }
    if (this.invuln > 0) {
